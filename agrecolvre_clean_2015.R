...
LucasS2015_dir <- "/Users/xavi_rp/Documents/LifeWatchERIC/LUCAS_SOIL_Data/LucasSoil_2015/"
LucasS2015 <- read.csv(paste0(LucasS2015_dir, "LUCAS_Topsoil_2015_20200323.csv"), header = TRUE)
...
LucasS2015_modified <- LucasS2015 %>%
  mutate(LC_modif_2015 = case_when(
    LC1 == "C21" ~ "C20",     
    LC1 == "C22" ~ "C20",     
    LC1 == "C23" ~ "C20",     
    LC1 == "C31" ~ "C30",     
    LC1 == "C32" ~ "C30",     
    LC1 == "C33" ~ "C30",     
    TRUE ~ LC1           # Keep other values unchanged
  ))
...
#sort(unique(LucasC2009$LC1))  # this is the one to check for changes 2009-2015
#sort(unique(LucasS2015_modified$LC1))   # this is the one to check for changes 2009-2015

## Selecting LUCAS-Soil 2018 that have the same LU than in 2009
LucasS2015_sameLU <- LucasS2015_modified %>%
  inner_join(LucasC2009 %>% select(POINT_ID, LC1), by = c("Point_ID" = "POINT_ID")) %>%
  rename(LC1_2009 = LC1.y)  %>%
  rename(LC = LC1.x)  %>%
  rename(POINTID = Point_ID)  %>%
  filter(LC_modif_2015 == LC1_2009) 
...
summary_table1 <- LucasS2015_sameLU %>%
...
# counting LUCAS points (unchanged 2009-2015) by region and by LC
desired_cols <- sort(unique(LucasS2015_sameLU$LC))

summary_table1_1 <- LucasS2015_sameLU %>%
  select(POINTID, NUTS_0, NUTS_1, NUTS_2, NUTS_3, LC) %>% 
  mutate(across(POINTID, as.character)) %>% 
...
save_as_docx(table_flex, path = "./results_2015/summary_table1.docx")
...
if(select_same_LU == "yes"){
  LucasS2015_sameLU_reg <- LucasS2015_sameLU %>%
  ...
  LucasS2015_sameLU_reg <- LucasS2015 %>%
    filter(if_any(everything(), ~ . %in% region_study)) %>% #View()
    #filter(grepl("^B|^C|^D|^E", LC)) 
    filter(grepl("^C|^D|^E", LC)) # forests, shrublands and grasslands 
} else{
...
LucasS2015_sameLU_reg

# selection of relevant variables for the study (those of the original study)
variables_lst <-c("pH.CaCl2.", "pH.H2O.", "EC", "OC", "CaCO3", "P", "N", "K")

LucasS2015_sameLU_reg_vrbls <- LucasS2015_sameLU_reg %>% 
  select(all_of(variables_lst)) %>%
  mutate(across(everything(), as.numeric)) %>%
  as_tibble()

LucasS2015_sameLU_reg_vrbls
...
pca_result <- prcomp(na.omit(LucasS2015_sameLU_reg_vrbls),   # to remove rows with NAs (remain 29 rows out of 102) --> probably it's too many
...
## saving plots
ggsave(paste0("./results_2015/pca_plot_", region_study, ".png"), plot = pca_plot, width = 8, height = 6, dpi = 300)
...
saveWorkbook(wb, paste0("./results_2015/pca_summary_", region_study, ".xlsx"), overwrite = TRUE)
...
LucasS2015_sameLU_reg_LC <- LucasS2015_sameLU_reg %>%
...
## boxplots
bxplt_variable_LC <- LucasS2015_sameLU_reg_LC %>% 
...
## saving boxplots
ggsave(paste0("./results_2015/bxplt_variable_LC_", region_study, ".png"), plot = bxplt_variable_LC, width = 12, height = 6, dpi = 300)



## Histograms
histog_variable_LC <- LucasS2015_sameLU_reg_LC %>% 
  pivot_longer(cols = all_of(variables_lst), names_to = "variable", values_to = "value") %>%
...
## saving boxplots
ggsave(paste0("./results_2015/histograms_variable_LC_", region_study, ".png"), plot = histog_variable_LC, width = 12, height = 12, dpi = 300)


# Variables are not normally distributed, in general.
## Quick check of variances
LucasS2015_sameLU_reg_LC %>%
  group_by(LC_grouped) %>%  
...
#                            function(var) {
#                              model <- aov(as.formula(paste(var, "~ LC_grouped")), data = LucasS2015_sameLU_reg_LC)
...
#                         function(var) {
#                           car::leveneTest(as.formula(paste(var, "~ LC_grouped")), data = LucasS2015_sameLU_reg_LC)
#                         })
#
...
## ANOVA
anova_results <- LucasS2015_sameLU_reg_LC %>%
...
kruskal_results <- LucasS2015_sameLU_reg_LC %>%
  select(-LC_grouped) %>%
  select(where(is.numeric)) %>%
  map_dfr(~ broom::tidy(kruskal.test(.x ~ LucasS2015_sameLU_reg_LC$LC_grouped)), .id = "Soil_Variable")
...
get_tukey_results <- function(var) {
  ...
  anova_result <- aov(formula, data = LucasS2015_sameLU_reg_LC)
  tukey_result <- TukeyHSD(anova_result)
  
  # Extract mean and CI
  summary_stats <- LucasS2015_sameLU_reg_LC %>%
    group_by(LC_grouped) %>%
    summarise(
...
ggsave(paste0("./results_2015/anova_means_tukey_", region_study, ".png"), plot = anova_means_tukey, width = 12, height = 6, dpi = 300)
...
saveWorkbook(wb, paste0("./results_2015/ANOVA_summary_", region_study, ".xlsx"), overwrite = TRUE)
