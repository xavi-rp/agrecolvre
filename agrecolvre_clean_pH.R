...
### step 5: Carbon Sequestration potential in relation to pH and LC ####

# Using soil Organic Carbon as a proxy: OC as an indicator of sequestration potential.
# 

# 1- Group by LC type (broadleaves, coniferous, shrublands, grassland)
# 2- Visualising: scatter plots and interaction plots to show OC vs. pH patterns across LCs
# 3- Analyzing trends: regression analysis to assess how OC changes along the soil pH gradient within each LC type.
# 4- Testing differences: ANOVA or linear models with interaction terms (OC ~ pH * LC) to test whether pH effects on OC differ between LCs.


## dataset
vrbles <- c("pH_CaCl2", "OC", "LC_grouped")

data_ph <- LucasS2018_sameLU_reg_LC %>%
  select(all_of(vrbles))

nrow(data_ph) # 589


## Scatter plot
ggplot(data_ph, aes(x = pH_CaCl2, y = OC, color = LC_grouped)) +
  geom_point(alpha = 0.7) +
  geom_smooth(method = "lm", se = FALSE) +
  labs(title = "OC vs. Soil pH (CaCl2) by Land Cover",
       x = "Soil pH (CaCl2)",
       y = "Soil Organic Carbon (OC)",
       color = "Land Cover") +
  theme_minimal()


## Interaction Plot (OC by binned pH_CaCl2 and LC_grouped)
# Bin the pH_CaCl2 variable
data_ph$pH_bin <- cut(data_ph$pH_CaCl2, 
                      breaks = seq(floor(min(data_ph$pH_CaCl2, na.rm = TRUE)),
                                   ceiling(max(data_ph$pH_CaCl2, na.rm = TRUE)),
                                   by = 0.5))

data_ph

# Calculate mean OC per pH bin and land use group
summary_data <- data_ph %>%
  group_by(pH_bin, LC_grouped) %>%
  summarise(mean_OC = mean(OC, na.rm = TRUE), .groups = "drop")

summary_data

# Create the interaction plot
ggplot(summary_data, aes(x = pH_bin, y = mean_OC, group = LC_grouped, color = LC_grouped)) +
  geom_line() +
  geom_point() +
  labs(title = "Interaction Plot: OC by pH Bin and Land Cover",
       x = "Soil pH Bin (CaCl2)",
       y = "Mean OC (%)",
       color = "Land Cover") +
  theme_minimal() +
